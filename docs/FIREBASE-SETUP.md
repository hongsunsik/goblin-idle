...
- 저장은 **전부 자동**입니다(저장 버튼 없음). 환생·전직·상점 구매 뒤 4초 안에, 평소에는 1분마다, 창을 가릴 때 올립니다. 다른 화면에 있다 돌아오면 다른 기기의 진행을 확인합니다. **바뀐 게 없으면 서버에 쓰지 않습니다.**
...
Spark(무료) 등급의 Firestore는 하루 쓰기 2만 회, 읽기 5만 회 정도입니다(정확한 값은 [요금 안내](https://firebase.google.com/pricing) 확인). 이 게임은 사용자 한 명당 접속 중 1분에 최대 1번(플레이 중에는 진행이 계속 바뀌므로 사실상 1분에 1번), 바뀐 게 없으면 0번 쓰고, 돌아왔을 때 확인하는 읽기가 가끔 있습니다. 하루 2시간 플레이하는 사용자라면 약 120번 쓰는 셈이라 소규모로는 넉넉하지만, 사용자가 수백 명이 되면 한도를 확인하세요.
