...
### 랭킹·출석 규칙 (Spark 요금제에서 됩니다)

`firestore.rules`에는 저장(`saves`) 말고도 **친선 랭킹(`ranks/{uid}`)** 과 **서버 출석(`attendance/{uid}`)** 규칙이 있습니다. 규칙을 게시하면 켜집니다(게시 전에는 게임이 "서버 규칙이 아직 켜져 있지 않아요"라고 안내합니다).

| 컬렉션 | 하는 일 | 규칙이 검사하는 것 |
|---|---|---|
| `ranks/{uid}` | 닉네임·최고 스테이지·누적 증표·업적 수 등 | 본인 문서만 쓰기, 필드와 값 범위, 기록은 줄어들지 않음, 30초에 한 번, 목록은 100개까지, 서버 시각(`updatedAt == request.time`) |
| `attendance/{uid}` | 출석 날짜·연속·누적 | 본인만, 하루 한 번, **날짜를 서버 시각으로 계산**(한국 시간), 연속·누적 계산이 맞는지 |

게시 후 확인: 게임에서 Google 연동 → 기록 탭 > 일일에서 **출석 체크**, 기록 탭 > 랭킹에서 **닉네임을 정하고 참여**. 둘 다 성공하면 규칙이 맞게 동작하는 것입니다. 랭킹은 처음 열 때 정렬용 인덱스가 필요하지 않습니다(필드 하나로만 정렬하므로 자동 인덱스).

> 이 규칙은 **에뮬레이터로 실행 시험을 하지 못했습니다**(이 개발 컴퓨터에 Java가 없음). 그래서 게시 직후 위 두 가지를 직접 눌러 보는 것이 가장 확실한 점검입니다. Firebase CLI로 게시하는 경우 `firebase deploy --only firestore:rules`.
