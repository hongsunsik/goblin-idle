...
(**증표** 탭) 환생으로 얻은 **왕의 증표**를 써서 **영구 강화**를 삽니다. 산 강화는 환생해도 남습니다. 증표를 써도 공격력·골드 +40%(지금까지 번 증표 기준)는 줄지 않습니다.
...
**증표 탭**이 왕의 증표로 사는 영구 강화이고, 별도의 **상점 탭**에서는 **크리스탈**로 물약·장비 상자·편의 상품을 삽니다.
...
| 상품 | 가격 | 효과 |
|---|---|---|
| 황금 / 힘 / 신속 물약 | 60 | 30분: 골드 +100% / 공격력 +50% / 공격 속도 +40% |
| 지혜의 물약 | 80 | 30분: 경험치 +100% |
| 행운의 물약 | 100 | 30분: 장비 드롭 확률 ×2 |
| 용사의 비약 | 200 | 1시간: 공격력·골드 +50% |
| 시간의 모래 | 30 | 모든 스킬 쿨타임 즉시 초기화 |
| 고급 장비 상자 | 100 | 장비 3개 · 고급 60% / 희귀 30% / 영웅 9% / 전설 1% |
| 영웅 장비 상자 | 300 | 장비 1개 · 영웅 85% / 전설 15% |
| 전설 장비 상자 | 900 | 전설 장비 1개 (확정) |
| 가방 확장 | 80 | 가방 +6칸 (최대 +18칸) |
| 시작 패키지 | 250 | 영웅 장비 + 황금·힘의 물약 (계정당 1번) |

- 물약은 다시 사면 시간이 늘어나고(최대 3시간), 게임을 꺼 둔 동안에도 줄어듭니다. 화면에 남은 시간이 표시됩니다.
- 장비 상자의 **등급 확률은 상품 카드와 구매 확인 창에 그대로 공개**됩니다. 장비 레벨은 지금까지 도달한 최고 스테이지 기준입니다. 산 장비는 자동 판매되지 않습니다.
- **광고 보고 크리스탈:** 광고를 끝까지 볼 때마다 **크리스탈 10개**, **하루 3번**(하루 최대 30개)(한국 시간 자정에 초기화). 중간에 그만두면 횟수가 줄지 않습니다. **기기 시계를 바꿔도 횟수를 늘릴 수 없습니다.** 오늘 날짜를 기기 시계가 아니라 사이트를 내려 주는 서버의 응답 시각(HTTP `Date` 헤더)으로 정하고, 서버 시각을 못 받는 상태(오프라인·로컬 파일로 열기)에서는 새 하루로 넘어가지 않습니다. 새 하루는 인터넷에 연결돼 있어야 시작됩니다.
...
- **밸런스를 시뮬레이션으로 조정했습니다.** 강화를 자동으로 사고 전직·환생하는 봇으로 몇 시간 치 플레이를 돌려 성장 곡선을 확인합니다(`node tools/simulate.js`).
  ...
  - 직업 경로가 서로 비슷하게 성장하는지도 확인합니다. 4차까지 가능한 32가지 경로를 40분씩 돌리면 평균 54.9(51.0~59.8)이고, 2차까지만 간 경우(평균 50.8)보다 4스테이지쯤 앞섭니다. 스킬을 넣기 전(평균 52.6)과 비교하면 스킬 16종이 더한 것은 약 2스테이지라 진행을 과하게 흔들지 않습니다. 경로 간 격차는 9스테이지(약 16%)로, 클래스 자체의 동료 배율이 큰 사령술사 계열이 스킬 도입 전부터 가장 강했습니다. **조건이 맞을 때만 쓰는 스킬**(처형·보스 사냥)은 덜 쓰이는 만큼 위력을 올려 보정했고, 동료 배율에 좌우되던 소환은 "내 초당 피해의 30%"로 바꿔 직업에 상관없이 일정하게 했습니다. 2차 전직 레벨을 30에서 20으로 낮춘 것도 이 시뮬레이션 결과입니다(30이면 도달하기 전에 대부분 환생해서 볼 수 없었습니다).
...
- **로직 테스트가 있습니다.** `node tools/test.js`(146개)가 업적, 4단계 직업 트리, 도감, 증표 상점, 크리스탈 상점(주문 중복 방지·상자 확률·물약·광고 하루 3번), 장비(등급별 확률·자동 장착·판매·변조 방어), 클라우드 동기화, 저장·불러오기, 환생, 오프라인 보상, 진행 속도 회귀를 화면 없이 점검합니다.
...
| `store.js` | 크리스탈 상점 데이터 (충전 상품, 물약, 장비 상자와 확률, 가방 확장, 시작 패키지, 광고 횟수) |
...
| `docs/` | README에 쓰는 스크린샷, [Firebase 설정 안내](docs/FIREBASE-SETUP.md), [결제·광고 안내](docs/PAYMENTS.md) |
