...
- **밸런스를 시뮬레이션으로 조정했습니다.** 강화를 자동으로 사고 전직·환생하는 봇으로 몇 시간 치 플레이를 돌려 성장 곡선을 확인합니다(`node tools/simulate.js`).
  ...
  - 직업 경로가 서로 비슷하게 성장하는지도 확인합니다. 4차까지 가능한 32가지 경로를 40분씩 돌리면 평균 52.6(49.8~56.5)이고, 2차까지만 간 경우(평균 50.1)보다 2~3스테이지 앞서는 정도로 3·4차가 진행을 과하게 흔들지 않습니다. 격차는 6스테이지(약 12%)입니다. 2차 전직 레벨을 30에서 20으로 낮춘 것도 이 시뮬레이션 결과입니다(30이면 도달하기 전에 대부분 환생해서 볼 수 없었습니다).
...
- **로직 테스트가 있습니다.** `node tools/test.js`(94개)가 업적, 4단계 직업 트리, 도감, 증표 상점, 장비(등급별 확률·자동 장착·판매·변조 방어), 클라우드 동기화, 저장·불러오기, 환생, 오프라인 보상, 진행 속도 회귀를 화면 없이 점검합니다.
