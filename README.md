...
**50가지 업적**이 7개 분류(처치·스테이지·성장·환생·직업·장비·기타)로 있습니다. 보스 처치, 누적 골드, 스킬 사용 횟수, 전설 장비, 직접 공격 횟수, 광고 시청 등 예전에는 없던 기록도 셉니다(환생해도 유지).

- 달성할 때마다 공격력·골드가 영구히 **+1%**(최대 +50%)입니다.
- **크리스탈 보상:** 업적마다 크리스탈 5~100개(50개를 전부 달성하면 1,200개)를 **기록 탭에서 받습니다**("받기" 또는 "모두 받기"). 받을 보상이 있으면 기록 탭에 알림 점이 뜹니다. 예전 저장에서 이미 달성한 업적도 보상을 받을 수 있습니다.
- 카드마다 진행도(예: 154 / 1.00K)와 보상이 보입니다. 시간이 지날수록 어려운 목표(스테이지 150, 4차 직업 32종, 100만 → 1조 골드 등)가 있어서, 봇 기준으로 환생을 10분씩 반복하면 약 7시간 뒤에 40개 정도를 달성합니다.
...
- **로직 테스트가 있습니다.** `node tools/test.js`(210개)가 업적(보상 받기·누적 기록), 4단계 직업 트리, 도감, 증표 상점, 크리스탈 상점(주문 중복 방지·상자 확률·물약·유물·광고 하루 3번), 환생 보상과 직업 공명, 밸런스 회귀, 장비(등급별 확률·자동 장착·판매·변조 방어), 클라우드 동기화, 자리를 비운 시간(서버 시각·시계 조작·한도·나눠 받기), 저장·불러오기, 환생, 오프라인 보상, 진행 속도 회귀를 화면 없이 점검합니다.
