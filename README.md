...
- **로직 테스트가 있습니다.** `node tools/test.js`(306개)가 업적(보상 받기·누적 기록), 4단계 직업 트리, 도감, 증표 상점, 크리스탈 상점(주문 중복 방지·상자 확률·물약·유물·광고 하루 3번), 환생 보상과 직업 공명, 밸런스 회귀, 장비(등급별 확률·자동 장착·판매·변조 방어), 클라우드 동기화, 자리를 비운 시간(서버 시각·시계 조작·한도·나눠 받기), 저장·불러오기, 환생, 오프라인 보상, 진행 속도 회귀를 화면 없이 점검합니다.
