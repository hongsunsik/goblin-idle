...
- **디자인 26종:** 이름마다 모양이 다릅니다. 무기 5종(몽둥이·단검·손도끼·장검·지팡이), 방어구 4종(가죽·쇠사슬·판금·로브), 액세서리 8종(반지·목걸이·팔찌·귀걸이·부적 등). 등급은 테두리 색과 빛으로 구분합니다.
...
- **수치:** 등급이 높을수록 크고 **±5% 무작위**이며(예전 ±15%는 같은 장비끼리 차이가 너무 커서 줄임), **높은 스테이지에서 얻을수록 커집니다**(스테이지 40에서 나온 장비는 스테이지 1의 약 2배). 전설 무기는 스테이지 40에서 공격력 +52% 안팎입니다.
...
- **분해와 장비 레벨:** 장비를 **분해**하면 **가루**가 나오고(등급·레벨·강화 단계가 높을수록 많음), 가루로 **장비 레벨을 올립니다**(+1 / +10 / 최대). 레벨이 오르면 수치도 같은 비율로 오르고, 처음 뽑힌 좋은 편차는 그대로 남습니다. 상한은 **내 최고 스테이지 + 100**(초월 한 단계마다 +100, 최대 999)이라서 예전에 얻은 좋은 장비를 지금 진행도 이상으로 끌어올려 계속 쓸 수 있습니다.
- **장비 잠금(🔒):** 장비 창에서 잠그면 판매·분해·선택/정리·강화·초월 재료에서 모두 빠지고, 언제든 다시 풀 수 있습니다.
- **장비 초월(✦1~✦5):** 15강을 채운 장비는 가루와 같은 칸·같은 등급 이상 장비 1개로 초월합니다(실패 없음). 단계마다 효과 +25%, 장비 레벨 상한 +100. 초월한 장비를 분해하면 쓴 가루의 60%를 돌려받습니다.
- **눈빛과 감정 표시:** `tools/goblin-fit.py`가 그림마다 주황 눈동자 위치를 찾아 두어(125장 중 두 눈 100장·한 눈 25장), 평소엔 눈동자가 가끔 반짝이고 강타할 때 붉게 번뜩, 보스전엔 붉게 타오르고, 레벨업·희귀 장비엔 금빛이 됩니다. 머리 위에는 상황에 맞는 감정 아이콘(AI 생성 8종)이 떠오릅니다 — 보스 등장 느낌표, 체력 낮을 때 땀방울, 레벨업·보스 처치 반짝임, 연속 처치 하트, 쓰러지면 어지러움, 평화로울 때 음표·투지.
- **캐릭터 크기·프로필 얼굴 맞춤:** AI 그림마다 날개·오라 때문에 고블린이 차지하는 비율과 얼굴 위치가 달라서, `tools/goblin-fit.py`가 그림 125장의 초록 피부로 머리 크기와 얼굴 위치를 재서(초록 불꽃 등에 속는 8장은 직접 지정) 전투 화면은 머리가 같은 크기로 보이게, 프로필은 얼굴 가운데를 잘라 씁니다.
- **전설 이상 전용 디자인 9종:** 용아검·폭풍의 창·영혼의 낫·용린 갑옷·천상의 로브·미다스의 반지·바람 날개 장식·전쟁의 뿔나팔·거인의 건틀릿(AI 생성, 요청 9번 약 0.045 pollen). '판매 대신 분해'를 켜면 자동 판매 대상이 골드 대신 가루가 됩니다. (봇 실측: 후반엔 10분 판마다 가루 약 600~800, 옛 신화 장비 30→140레벨은 약 2,600)
...
왕의 증표로 사는 영구 강화입니다. 1~3단계는 공격력·골드·체력 말고도 **경험치, 공격 속도, 보스 피해, 받는 피해 감소, 체력 회복, 부활 시간, 스킬 쿨타임, 환생 증표, 분해 가루, 가방 칸, 희귀 이상 장비 확률**을 올리는 강화가 있고, 4단계에는 **장비 강화 성공 확률·던전/탑 크리스탈**, 5단계에는 끝없이 올리는 **공격력·골드 / 최대 체력 / 동료 공격**이 있습니다. 유물·특별 옵션과 같은 효과는 함께 더해지고 같은 상한(쿨타임 60%, 받는 피해 70% 등)을 받습니다. 봇 시뮬레이션으로 성장 속도가 예전과 같은 것(10판째 141)을 확인했습니다.
...
- **장비 상점 (시간마다 갱신):** 특별 옵션이 하나 붙은 **영웅·전설 장비 6개**(무기·방어구·액세서리 2개씩, 전설은 약 25%)가 진열되고, **4시간마다 새 물건으로 바뀝니다**. 남은 시간이 화면에 보이고, 크리스탈 40개로 **새로고침**할 수도 있습니다(한 갱신 시간에 3번까지). 진열은 (시간 구간, 새로고침 횟수, 내 최고 스테이지)만으로 계산하므로 언제 열어도 같고, 구간은 광고 횟수와 같은 방식으로 **서버 시각**으로 셉니다(기기 시계를 돌려도, 서버 시각을 못 받아도 새 물건이 나오지 않음).
  ...
  - 장비 이름 앞에 옵션 이름이 붙습니다(예: **사냥꾼의** 고귀한 장검). 수치는 드롭(±5%)과 달리 기본값 이상(+0~5%)으로 나오고, **옵션 없는 드롭에게 자리를 뺏기지 않으며** 옵션 있는 쪽은 수치가 90%만 돼도 자리를 얻습니다. 가방 "정리"·자동 판매 대상도 아닙니다. 장비 탭과 장비 창에 ★로 옵션이 보이고, 낀 장비의 옵션은 유물과 합쳐집니다(쿨타임 감소 60%·받는 피해 감소 70% 같은 상한이 있습니다).
...
- **옆으로 달려가는 전투 화면**(레전드 오브 슬라임 같은 느낌): 지역마다 하늘·먼 산·중간·가까운 땅 **4겹 배경**(AI 생성 24장, 약 0.12 pollen, 투명 배경)이 멀수록 느리게 흐르고(패럴랙스), 층마다 원본·좌우 반전본을 번갈아 이어 붙여 이음새가 없으며, 발밑 풀 띠는 더 빠르게 흘러 깊이감이 납니다. 몬스터를 잡으면 잠깐 배경이 빠르게 흐르며 고블린이 통통 뛰어 달리고, 다음 몬스터가 오른쪽에서 걸어 들어옵니다. 싸우는 동안에도 배경은 바람처럼 아주 천천히 흐릅니다.

- **일반 몬스터 48종**: 지역 6곳마다 8종, 모두 그 지역 전용 그림(AI 생성, 요청 50번 약 0.25 pollen)입니다. 예: 숲의 버섯 요정·새싹 나무정령·왕벌, 동굴의 수정 게·두더지 광부, 사막의 미라·선인장 전사, 얼음 산맥의 펭귄 기사·아기 설인, 화산의 샐러맨더·마그마 사냥개, 저주받은 성의 살아있는 갑옷·저주 인형. 같은 지역을 다시 돌 때(II·III회차)는 등장 순서가 밀려 다른 조합으로 나옵니다.
...
- **난이도는 '이번 판의 최고 스테이지' 기준**입니다(보상 장비 레벨·골드도 같은 기준). 예전에는 평생 최고 스테이지 기준이라 환생 뒤에는 거의 못 깼습니다(장기 봇 실측: 15일 뒤 완주 가능한 순간 0%). 지금은 15일 이후에도 일일 약 80%·주간 약 70%·월간 약 60%의 순간에 미니게임 없이 완주할 수 있고, 미니게임을 하면 80~90%입니다.
- **미니게임**: 3·2·1 카운트다운, 효과음(적중·완벽·실패·콤보), 끝나면 S/A/B/C 등급과 보너스 %를 보여 줍니다. 두더지 잡기는 9칸 흙더미 구멍에 콤보(3콤보마다 +1), 타이밍 게이지는 라운드마다 빨라지고 노란 구간이 옮겨 가며, 패턴 반격은 보스가 던지는 번개를 방패가 빛나는 순간 되받아칩니다.
...
- 음악 파일 없이 **Web Audio로 코드가 직접 연주하는 칩튠**입니다(`audio.js`, 음표 데이터만 들어 있음). **지역 6곳마다 다른 곡**(숲=경쾌한 장조, 동굴=신비로운 단조, 사막=프리지안, 얼음 산맥=종소리, 화산=질주하는 단조, 저주받은 성=오르간)과 **보스 곡·던전/탑 전투 곡**, 총 8곡이고 마디가 바뀔 때 부드럽게 넘어갑니다. 배음을 줄인 부드러운 음색, 긴 음 비브라토, 에코·잔향, 압축기를 씁니다. 곡마다 크기는 소리 없이 그려서 재 보고 맞췄습니다(ui-check).
- 브라우저 규칙상 첫 터치 뒤에 시작하고, 앱이 가려지면 멈춥니다. 설정(⚙)에서 켜고 끄기·음량을 바꿀 수 있습니다.
- **효과음**: 공격 방식 13종마다 다른 무기 소리 + 몬스터가 맞는 '퍽' 소리, 처치·레벨업·장비 드롭(등급별)·쓰러짐·보스 등장 소리. 배경음과 따로 켜고 끄고 음량을 조절합니다.
...
- **직업 속성 타격 23종:** 125개 직업마다 속성(화염·얼음·번개·신성·공허·폭풍 등)을 하나씩 정해서, 공격 모양 위로 속성 그림이 피어납니다. 부모·자식·형제 직업끼리는 속성이 겹치지 않아 같은 계통이어도 직업마다 다르게 터집니다(테스트로 검사).
- **근접 휘두름 궤적 5종**(칼·도끼·성검·망치·단검)은 회전 없이 휘두르는 방향으로 그려지듯 나타나고, **타격 이펙트 26종**은 공격 방식 13종 × 기본(1~3차)·화려한(4~5차, 마법진·빛줄기) 그림입니다. 전직할수록 커지고, 5차는 한 겹 더 겹칩니다.
...
- **로직 테스트가 있습니다.** `node tools/test.js`(314개)가 업적(보상 받기·누적 기록), 4단계 직업 트리, 도감, 증표 상점, 크리스탈 상점(주문 중복 방지·상자 확률·물약·유물·광고 하루 3번), 환생 보상과 직업 공명, 밸런스 회귀, 장비(등급별 확률·자동 장착·판매·변조 방어), 클라우드 동기화, 자리를 비운 시간(서버 시각·시계 조작·한도·나눠 받기), 저장·불러오기, 환생, 오프라인 보상, 진행 속도 회귀를 화면 없이 점검합니다.
...
- **몇 년 플레이를 기준으로 밸런스를 맞췄습니다.** `tools/balance.js`는 하루 1~2시간 접속하는 봇(오프라인 보상·던전·탑·장비 상자·가루·강화·초월·증표 상점 포함)으로 몇 달~몇 년을 돌려 날짜별 스테이지·환생·증표·크리스탈·가루·장비를 봅니다. 여러 후보 공식을 1년·2년치로 비교해서 정했습니다.
  - 예전에는 첫날 2시간에 스테이지 160까지 치솟고(10분 차에 월간 던전 완주 등), 3달 뒤에는 하루 +0.08 스테이지로 사실상 멈췄습니다.
  - 지금은 **첫날 약 130 → 3일 약 190 → 1년 약 530~650 → 2년 약 660~810**, 반년 뒤에도 하루 +0.4~0.5씩 꾸준히 오릅니다(하루 1시간 기준).
  - 바꾼 것: 던전·탑 입장 조건, 스테이지 120·300부터 몬스터 체력 증가 완화(×1.18 → ×1.075, 강화 성장과 거의 맞먹게; ×1.06 이하는 폭주), 환생 증표 곡선(최고 스테이지^1.3÷50), 증표 효과를 개수의 1.15제곱으로, 레벨 60 이후 경험치, 끝없는 증표 강화 최대 1,000레벨, 저장 상한 상향(스테이지 99,999 등).
  - **직업 격차**: 체력 배율이 높은 직업이 공격력도 얻는 '투지'와, 4·5차 직업 공격력 배율을 봇으로 반복 보정해서 5차 64종의 실질 전투력 차이를 5.1배 → 1.7배로 줄였습니다.
...
| `tools/goblin-fit.py` | 고블린 그림마다 머리 크기·얼굴 위치를 재서 `images/goblin-fit.js`를 만듦 (새 그림을 넣으면 다시 실행) |
| `audio.js` | 배경음 (Web Audio 칩튠, 음표 데이터) |
| `tools/balance.js` | 장기 밸런스 봇 (며칠~몇 년, `DT`·`ONLY`·`OFF`·`GAME` 옵션으로 빠르게·나눠서·기능 끄고·다른 공식으로) |
