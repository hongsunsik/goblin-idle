...
직업마다 능력치 배율이 달라서 성장 방향이 바뀌고, 고블린의 모습도 직업에 맞게 바뀝니다.

#### 직업 도감

2차 전직을 달성한 직업은 **도감**에 기록되고, 하나당 공격력·골드가 영구히 +5%씩 오릅니다. 도감은 1차 직업별로 묶여 있고, 카드를 누르면 자세한 정보가 나옵니다.

- **기록:** 그 직업으로 도달한 **최고 스테이지**, **처치 수**, **전직 횟수**
- **등급:** 최고 스테이지 20 / 35 / 50에 도달하면 동·은·금 메달을 받고, 등급 하나마다 그 직업의 보너스가 +1%p 늘어납니다. (직업 하나의 최대 보너스는 +8%)
- 아직 만나지 못한 직업은 실루엣과 "어느 직업에서 전직하는지" 힌트만 보입니다.
...
- **전투 모션:** 고블린은 웅크렸다가(예비 동작) 돌진하고 멈칫했다가 돌아오며, 몬스터는 닿는 순간 밀려나고 하얗게 번쩍입니다. 쓰러질 때는 튕겨 날아가며 사라지고 동전이 튀며, 새 몬스터는 통통 튀며 나타납니다. 보스 등장은 붉은 번쩍임과 화면 흔들림, 레벨업은 금빛 고리, HP 바는 깎인 만큼 잔상이 남았다가 따라옵니다. 숫자는 종류마다 뜨는 자리를 나눠서 겹치지 않게 했습니다.
...
- **밸런스를 시뮬레이션으로 조정했습니다.** 강화를 자동으로 사고 전직·환생하는 봇으로 몇 시간 치 플레이를 돌려 성장 곡선을 확인합니다(`node tools/simulate.js`).
  ...
  - 8가지 직업 경로가 30분 기준 서로 5스테이지 안에서 비슷하게 성장하는지도 확인했습니다(45~50). 2차 전직 레벨을 30에서 20으로 낮춘 것도 이 시뮬레이션 결과입니다(30이면 도달하기 전에 대부분 환생해서 볼 수 없었습니다).
...
- **로직 테스트가 있습니다.** `node tools/test.js`(49개)가 업적, 직업 도감, 증표 상점, 장비(등급별 확률·자동 장착·판매·변조 방어), 저장·불러오기, 환생, 오프라인 보상, 진행 속도 회귀를 화면 없이 점검합니다.
...
| `tools/ui-check.js` | 실제 브라우저에서 눌러 보는 점검 (탭 공격 모션, 강화·상점 구매, 도감·장비 창, 전설 드롭 연출, 브라우저 오류) |
