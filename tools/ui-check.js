...
(async () => {
  ...
  try {
    ...
    check('도전 확인 창에 파동별 보스 목록(3줄)과 피해 예산이 보인다', (await ev(`document.querySelectorAll('#modalBody .dwrow').length`)) === 3 && (await txt('#modalBody')).includes('피해 예산'));
    await shot('dungeon-confirm');
    check('카드에 예상 결과(완주 가능)가 보인다', (await txt('#dungeonList')).includes('완주 가능'));
    await click('#modalActions .btn--gold'); await sleep(300);
    ...
    check('미니게임이 끝나면 보스 전투 연출이 나온다', !(await ev(`document.getElementById('mgModal').hidden`)) && !!(await ev(`document.querySelector('#mgStage .dfight__boss img, #mgStage .dfight__boss svg')`)));
    await sleep(900); await shot('dungeon-fight');
    check('전투 연출 중 보스 체력바가 줄어든다', parseFloat(await ev(`document.getElementById('dfHp').style.width`)) < 100 || (await ev(`document.querySelectorAll('#dfLog .dfight__row').length`)) > 0);
    await click('#mgSkip'); await sleep(300);
    check('미니게임을 건너뛰어도 충분히 강하면 파동을 모두 물리치고 완주 결과 창이 뜬다', (await txt('#modalTitle')).includes('완주'));
    await shot('dungeon-result');
    ...
    await click('#modalActions .btn--gold'); await sleep(300);
    await sleep(700); await shot('dungeon-mole');
    ...
    await click('[data-dchallenge="daily"]'); await sleep(300);
    check('완주한 던전은 도전 창에 소탕 버튼이 생긴다', (await ev(`[...document.querySelectorAll('#modalActions .btn')].some((b) => b.textContent === '소탕')`)));
    await ev(`[...document.querySelectorAll('#modalActions .btn')].find((b) => b.textContent === '소탕').click()`); await sleep(300);
    check('소탕은 미니게임 없이 바로 전투 연출로 간다', !(await ev(`document.getElementById('mgGrid')`)) && !!(await ev(`document.getElementById('dfArena')`)));
    await click('#mgSkip'); await sleep(300);
    check('소탕 결과 창에 소탕 보너스가 표시된다', (await txt('#modalBody')).includes('소탕 보너스'));
    await click('#modalActions .btn'); await sleep(300);
    check('소탕도 도전 횟수를 쓴다 (1/3)', (await txt('#dungeonList')).includes('1/3'));
    ...
    const x0 = await ev(`new DOMMatrix(getComputedStyle(document.querySelector('.plx--near')).transform).m41`); await sleep(1500);
    const x1 = await ev(`new DOMMatrix(getComputedStyle(document.querySelector('.plx--near')).transform).m41`);
    check('배경이 옆으로 흐른다 (왼쪽으로 이동)', x1 < x0, `${x0} → ${x1}`);
    check('층별 배경(하늘·먼 산·중간·가까운 땅)이 보이고, 가까운 층이 먼 층보다 빨리 흐른다', await ev(`(async () => {
      const x = (l) => new DOMMatrix(getComputedStyle(document.querySelector('.plx--' + l)).transform).m41 / document.querySelector('.plx--' + l).offsetWidth;
      if (getComputedStyle(document.getElementById('plx')).display === 'none') return false;
      const a = { far: x('far'), near: x('near') }; await new Promise((r) => setTimeout(r, 1500));
