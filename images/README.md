...
- **몬스터 지역 그림**: `golem.png` 하나만 있으면 모든 지역에서 그걸 쓰되, 지역 그림이 없으면 지역마다 색 필터(`art.js`의 `BIOME_TINT`)가 씌워져 색이 달라집니다. 지역 그림(`golem_3.png` 등)을 넣으면 필터 없이 그 그림이 그대로 나옵니다. 지역마다 다르게 하고 싶을 때만 `golem_2.png`(사막), `golem_3.png`(얼음)처럼 추가하세요. 지역 번호는 0 숲, 1 동굴, 2 사막, 3 얼음, 4 화산, 5 성입니다.
