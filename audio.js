...
(function (root) {
  ...
  const SONGS = {
    forest: {   // 고블린 숲: 경쾌한 장조
      bpm: 116, lead: 'soft', back: 'arp', drums: 'pop',
    ...
    cave: {   // 어둠의 동굴: 느리고 신비로운 단조, 음이 드문드문
      bpm: 88, lead: 'flute', back: 'pad', drums: 'soft',
    ...
    desert: {   // 불타는 사막: 이국적인 프리지안
      bpm: 104, lead: 'soft', back: 'arp', drums: 'hand',
    ...
    snow: {   // 얼음 산맥: 잔잔한 종소리
      bpm: 94, lead: 'bell', back: 'pad', drums: 'soft',
    ...
    castle: {   // 저주받은 성: 어두운 오르간
      bpm: 98, lead: 'organ', back: 'pad', drums: 'soft',
    ...
    boss: {   // 보스 스테이지: 빠르고 긴장감 있는 단조
      bpm: 146, lead: 'saw', back: 'drive', drums: 'rock',
    ...
    battle: {   // 던전·탑 전투: 영웅적인 단조
      bpm: 150, lead: 'soft', back: 'drive', drums: 'rock',
  ...
  function schedule() {
    ...
    while (nextT < ctx.currentTime + 0.15) {
      if (step % 8 === 0 && want !== song) {   // 곡은 마디가 바뀔 때, 잠깐 줄였다가 다시 키우며 넘어간다
        song = want; step = 0;
  ...
  root.GoblinAudio = {
    ...
    songFor(biome, boss, battle) { return battle ? 'battle' : boss ? 'boss' : BIOME_SONG[((biome % BIOME_SONG.length) + BIOME_SONG.length) % BIOME_SONG.length]; },
    get on() { return on; },
